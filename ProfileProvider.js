    ...
    getChildren(profile) {
        if (!profile) {
            ...
            if (profileName) {
                ...
                this.statusBarItem.tooltip = `Current default AWS profile is ${profileName}`;
                this.statusBarItem.show();
            } else {
                this.statusBarItem.hide();
            }

            const profilesFiltered = profiles.filter(profile => profile.sts || this.mustShowHardToken);

            let items = profilesFiltered.map(profile => new TreeProfile(profile, profile.display, vscode.TreeItemCollapsibleState.Collapsed));
            return items.sort((a, b) => a.profile.name.localeCompare(b.profile.name));
